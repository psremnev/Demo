...
// для Демо страницы
setInterval(() => {
  serverChannel.send('eventone', {msg: 'You get data because subscribe on channel'})
}, 1000)
...
// обработка роутинга
app.get(/^\/(?!service|favicon)(\w+)?/, async(req, res) => renderPage(req, res));
...
app.post(appConfig.service_address, function (req, res) {
  ...
  const getData = () => {
    ...
    return isList ? listData.reverse() : treeData.filter((item) => {
      if (!params.parent) {
          return true;
      }
      const parentIsArray =  Array.isArray(params.parent);
      // отдаем корневые узлы !item.parent и узлы из parent иначе узлы из parent
      return parentIsArray ? (params.parent.includes(item.parent) || !item.parent) : params.parent === item.parent;
    }).reverse();
  }
  res.send(getData());
});
