...
export default function Header() {
    ...
    const langItems = useMemo(() => [{ id: LANG.RU, title: 'Ru' }, { id: LANG.ENG, title: 'En' }], []);
    const themeItems = useMemo(() => [{ id: THEME.light, title: 'Light' }, { id: THEME.dark, title: 'Dark' }], []);
    ...
    const paramChanged = (selectedKeys, lang = true) => {
        ...
        const param = (lang ? langItems : themeItems).filter((item) => item.id === selectedKey)[0].id;
        ...
        <header className="app-header">
            <section className="app-header__firstRow">
                ...
                <section className="app-header__firstRowRight">
                    <Contacts vk='https://vk.com/p.remnev' telegram='https://t.me/Hhappy_47' />
                    <Settings items={settingsItems}/>
                </section>
            </section>
            <section className="app-header__secondRow">
                { accordion.map((item, index) => 
                    <div key={index} className='app-header__accordionBtn marginRight-pre-m'>
                        <Button backgroundColor='var(--second_color)'
                                title={item.title}
