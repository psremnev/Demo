...
export default function HintTemplate({
  ...
  return (
    <div style={style} className="hintTemplate">
      {imageSrc && (
        <img
          className={`hintTemplate__image ${
            orientation === ORIENTATION.HORIZONTAL ? 'marginRight-s' : ''
          }`}
      ...
      <section
        style={styleText}
        className={`hintTemplate__textBlock ${
          orientation === ORIENTATION.HORIZONTAL ? 'marginRight-s' : ''
        }`}
      >
        <span className="hintTemplate__title marginRight-s">{title}</span>
        {additionalText && (
          <span className="hintTemplate__additionalText">{additionalText}</span>
        )}
      </section>
    </div>
  );
