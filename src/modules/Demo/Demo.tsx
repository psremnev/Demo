...
export default function({preloadData}) {
    ...
    const openPopup = (type: string, options?: Partial<IPopupOptions>) => {
        ...
        dialog.open({
            ...{
                ...
                title: `Header ${type}`,
                width: 300,
                content: () => {return <div>{ `Hi, i m content node from ${type}`}</div>}
            },
            ...options
        });
    }
    ...
    const channelSubscribe = () => {
        isSubscribe ? channel.unsubscribe('eventone') : channel.subscribe('eventone', (msg) => console.log(msg));
    ...
    const carouselItemTemplate = ({item}) => {
        return <div>
                    <img src='https://cdnn21.img.ria.ru/images/148839/96/1488399659_0:0:960:960_600x0_80_0_1_e38b72053fffa5d3d7e82d2fe116f0b3.jpg'
                            style={ {height: '200px', width: '200px', borderRadius: 10, marginRight: 4} }/>
                        {item.title}
                </div>
    }

    return (
        <div style={ {display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: '100%', padding: 6,boxSizing: 'border-box'} }>
            ...
            <br />
            <Carousel items={preloadData.list}
                      source={new Service({endpoint: 'List'})}
                      itemTemplate={carouselItemTemplate}/>
            <br />
            <Header title='Audio'/>
            <br />
            <audio controls><source src="./public/1.mp3" type="audio/mpeg" /></audio>
            <br />
            ...
            <br />
            <Button onClick={() => openPopup(POPUP_TYPE.DIALOG, {confirmationCfg: {
            ...
            <br />
            <Button onClick={btnClick} title="Base with background" backgroundColor='rgb(229 218 254)' />
            <br />
            ...
            <br />
            <div className='demoList'>
                <TreeList items={preloadData.tree}
                          itemsContainerPadding={itemsContainerPadding}
            ...
            <br />
            <Button onClick={channelSubscribe} title={isSubscribe ? 'Unsubscribe' : 'Subscribe'} />
            <br />
            <Header title='SelectorList'/>
            <br />
            <SelectorList items={preloadData.list} />
            <br />
            <SelectorList items={preloadData.list} multiSelect={false} selectedKeys={[1]}/>
            <br />
            <Header title='HintTEmplate'/>
            <br />
            <HintTemplate imageSrc='public/header.gif' title='HEADER' additionalText='add text'/>
            <br />
            <HintTemplate orientation='horizontal' imageSrc='public/header.gif' title='HEADER' additionalText='add text'/>
            <br />
            <HintTemplate orientation='horizontal' imagePosition='end' imageSrc='public/header.gif' title='HEADER' additionalText='add text'/>
            <br />
        </div>
    )
}