...
export default function FileLoader({
    ...
    const style = {
        dropArea: {
            ...
            borderRadius: 4,
            alignItems: 'center',
            background: isDropEnter ? 'var(--light_grey_background_color)' : 'var(--default_background_color)',
            padding: 6,
    ...
    return (
        <section className={`fileLoader__type-${type}`} style={ type === FileLoaderType.BUTTON ? style.button : {} }> 
            {type === FileLoaderType.BUTTON ?
                <>
                    <label className='ti-upload cursorPointer' htmlFor="fileLoad" />
                    <input className='displayNone' type="file" id="fileLoad" onChange={onChange} multiple={multiSelect}/>
                </>
                :
