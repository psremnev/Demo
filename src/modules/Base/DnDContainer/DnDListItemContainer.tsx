import {IDnDListItemContainer, IDragElement} from "DnDContainer/IDnDListItemContainer";
...
export default function DnDListItemContainer({
    ...
    const insert = (position = POSITION.AFTER) => {
        ...
        if (dragElement) {
            position === POSITION.AFTER ? container.after(dragElement) : container.before(dragElement);
        }
    }
    ...
    const onDragStart = async(e: DragEvent) => {
        if (canDrag) {
            ...
            globalThis.dragElement = {
                element: containerEl,
                options,
                positionEl: {after: containerEl.previousSibling, before: containerEl.nextSibling, parent: containerEl.parentElement},
